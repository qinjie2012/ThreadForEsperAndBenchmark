 ...
 * 针对每个语句提供语句的生命周期的所有service  包括创建语句后开始。
 */
public class StatementLifecycleSvcImpl implements StatementLifecycleSvc
{
     ...
     */
    protected final EPServicesContext services;//里面也有着一个statement lifecycle management的实体，不会导致循环依赖吗？
    ...
    private final Set<StatementLifecycleObserver> observers;//目前观察，observer好像与JMS系统有关联
    ...
    public void addObserver(StatementLifecycleObserver observer)//观察者模式
    {
    ...
    /**
     * Create stopped statement.刚创建的默认为stop的状态
                                                         ...
                                                         EPStatementObjectModel optionalModel)
    {
        ...
        services.getStatementEventTypeRefService().addReferences(statementName, compiledSpec.getEventTypeReferences());//关联了事件类型的引用？
        ...
        try
        {
            ...
            long timeLastStateChange = services.getSchedulingService().getTime();   //关联语句创建的时间，对应后面针对语句的生命周期
    ...
    public void dispatchStatementLifecycleEvent(StatementLifecycleEvent theEvent)
    {
        ...
        {
            observer.observe(theEvent);   //观察对应的EPL语句，注册查询的关键？
        }
    }
