...
public class StreamFactorySvcImpl implements StreamFactoryService
{
    ...
    {//这个方法是在后面触发查询的时候也会调用此方法中实现的matchFound方法
        ...
        {
            filterCallback = new FilterHandleCallback()//创建语句的时候在这次创建了对应的CallBack
            {
                public String getStatementId()
                {
                    return statementId;
                ...
                public void matchFound(EventBean theEvent, Collection<FilterHandleCallback> allStmtMatches)
                {
                    ...
                    eventStream.insert(theEvent);   //触发了每个eventStream的newDataBuff的更新操作
                    if (InstrumentationHelper.ENABLED) {
