...
public class MarketClient extends Thread {
    ...
    public MarketClient(Client client) {
        ...
        for (int i = 0; i < market.length; i++) {
            market[i] = new MarketData(Symbols.SYMBOLS[i], Symbols.nextPrice(10), Symbols.nextVolume(10), Country.nextCountry("CN"));
        }
        ...
        try {
            ...
            do {
                ...
                for (int i = 0; i < eventPer50ms; i++) {
                    ...
                    byteBuffer.clear();//为buffer中放数据做好准备
                    md.toByteBuffer(byteBuffer);
                    byteBuffer.flip();//为从buffer中取数据做好准备
