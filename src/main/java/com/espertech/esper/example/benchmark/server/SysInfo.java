...
public class SysInfo {
    ...
    public void testCpuPerc() {
       ...
       /* CpuPerc cpu;
        ...
        for (int i = 0; i < cpuList.length; i++) {
        	//System.out.println("第"+(i+1)+"个CPU使用情况：");
    ...
    private void printCpuPerc(CpuPerc cpu) {
    	System.out.print(CpuPerc.format(cpu.getCombined())+" ");//cpu使用率
    ...
    public void getPhysicalMemory() {
        ...
        try {
            mem = sigar.getMem();
            System.out.print(100 * mem.getUsed()/mem.getTotal() + "% ");//内存使用率
            // 内存总量
           /* System.out.println("内存总量：" + df.format((float)mem.getTotal() / 1024/1024/1024) + "G");
            ...
            // 当前内存使用率
            System.out.println("当前内存使用率： " + 100 * mem.getUsed()/mem.getTotal() + "%");
